...
public class Adapter extends RecyclerView.Adapter{
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, final int i) {
        ((MyviewHolder)viewHolder).textView.setText(list.get(i).getId());
        ViewCompat.setTransitionName(  ((MyviewHolder)viewHolder).talk_o, "one");
        mConversation = EMClient.getInstance().chatManager().getConversation(list.get(i).getId(), null, true);
        // 设置当前会话未读数为 0
        ...
        if (mConversation.getAllMessages().size() > 0) {
            ...
            if(messge.getBody() instanceof EMImageMessageBody){
                ((MyviewHolder)viewHolder).tel_itemmessage.setText("你收到了一张图片");
                ...
                ((MyviewHolder)viewHolder).tel_itemmessage.setText("收到一条语音");
            }
        }
